...
$(document).ready(function(){
	...
	$("a[data-name='modify']").click(function(){
        ...
        var gid=$("#gid").val();
		loadPage(ctxPath + "org/to-update-org?OrgId=" + orgId+"&orgName="+orgName+"&status="+status+"&gid="+gid);
	});
	/** 查看下属学院 */
	$("a[data-name='check']").click(function(){
		var fid = $(this).attr("data-orgId");
		loadPage(ctxPath + "aca/to-check-aca?fid=" + fid);
	});
	/** 预览 */
    ...
    $("a[data-name='Status']").click(function(){
        ...
        ajaxRequest(ctxPath + "org/toUpdateStatus", params, function(data){
            if(data.code==1){
...
function toSelect(gid) {
    layer.open({
        type: 2,
        title: '选择机构',
        shade: 0.8,
        area: ['700px', '325px'],
        content: ctxPath + "/admin/org-group/to-select-org?qGid=" + gid
    });
}
...
function query(params) {
    ...
    var name=divId.find("input[class='orgName']").val();
    divId.find("dl[class='query-list']").css('display','block');
    var htmls = "";
    $.getJSON("/front/org/query", {query: name}, function (data) {
        if(data.data.list.length>0){
            $.each(data.data.list, function (index, org){

                htmls += '<dd><a href="javascript:;" onclick="chooseOrg(' + params + ',' + org.fid + ',\'' + org.orgName + '\')">' + org.orgName + '</a></dd>';
                console.log(htmls)
            });
        }else{
        ...
        if(params==1){
            $("#resId .query-list").html(htmls);
            $("#resId .query-list").addClass("is-show");
        }
        if(params==2){
            $("#actId .query-list").html(htmls);
            $("#actId .query-list").addClass("is-show");
        }
        if(params==3){
            $("#carId .query-list").html(htmls);
            $("#carId .query-list").addClass("is-show");
        }
    });
}

 ...
 * @param orgId
 */
function initResClassifyData(){
	var loadResClassifiesUrl = ctxPath + "/admin/teach-material-type/list-datas";
	var params = {};
	ajaxRequest(loadResClassifiesUrl, params, function(response){
		$("#total").html(response.length);
		initZtree(response);
	}, function(){
		showError("加载教材分类列表出错.");
	});
}
/**
 * 初始化Ztree
			...
			addHoverDom: function(treeId, treeNode){
				...
				if(!treeNode.bottom){
					if($("#addBtn_"+treeNode.tId).length < 1){
						var addStr = "<span resClassifyId='"+ treeNode.id +"' class='button add' id='addBtn_" + treeNode.tId + "' title='新增教材分类' onfocus='this.blur();'></span>";
						sObj.after(addStr);
				...
				if(treeNode.bottom){//教材列表管理页面
                    if($("#resBtn_"+treeNode.tId).length < 1){
                        ...
                        $("#resBtn_"+treeNode.tId).click(function(){
                            var resClassifyId = $(this).attr("resClassifyId");
                            loadPage(ctxPath + "/admin/teach-material/show/"+resClassifyId, "", {"resClassifyId": resClassifyId});
                        });
                    }
				}
			},

...
function initForm(){
	validator = $("#domainRecommendRes_form").validate({
		rules:{
	...
	$("#domainRecommendRes_form").on("submit", function() {
		ajaxForm(this, function(data) {
            if(data.success){
            	showSuccess("域推荐资源excel导入成功", function(){
            		closeAllLayer();
            		loadPage(ctxPath + "/domainRecommendRes", "", {"domainId": $("#domainId").val()});
            	});
            }else if("error" == data.msg){
            	showError("域推荐资源excel导入出错.");
            }else if("failure" == data.msg){
            	showError("域推荐资源excel导入失败.");
            }else{
            	showError(data.msg);
            }
        }, function(){
...
function initWebUploader(){
	...
	uploader = WebUploader.create({
        ...
        // 文件接收服务端。
		server: ctxPath + '/domainRecommendRes/uploadDomainRecommendExcel',
        // 选择文件的按钮。可选。

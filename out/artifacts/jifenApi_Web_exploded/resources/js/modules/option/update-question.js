...
$(document).ready(function () {
    ...
    $("#reRes_form").on("submit", function() {
        ajaxForm(this, function(data) {
            if(data.code==1){
                showSuccess("修改题目成功", function() {
                    closeAllLayer();
                    loadPage(ctxPath + '/admin/option/system-list', null, {
                        "reResName": $("#reResName").val(),
                        "id": $("#resId").val()
                    });
                })
            }else {
    ...
    $("#save").click(function(){
        ...
        if(validator.form()) {
            $("#reRes_form").attr("action", ctxPath + "/admin/option/update-question");
    ...
    $("#addFile").click(function () {
        ...
        if(tem<4) {
            $(this).parent().before("<div class=\"controls\" >\n" +
                ...
                "                            <input type=\"hidden\" name=\"annex\" value=\"${item.fileName}\" />\n" +
